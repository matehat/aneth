(function() {
  ...
  prg.version(pkg.version).option('-w, --watch', 'Watch for advertised hosts and apply updates to /etc/hosts (requires sudo)').option('-s, --service <serviceName>', 'Service name to use in advertising', 'org-aneth-hosts').option('-p, --port <port>', 'Port to use for exchanges', parseInt, 4321).option('-i, --interval <ms>', 'Interval at which to pause and restart the service browser (when watching)', parseInt, 10 * 60 * 1000).option('-d, --delay <ms>', 'Delay before restarting the service browser (when watching)', parseInt, 50);

  prg.command('start [hostname]').description("Start the discovery service.").action(function(hostname) {
    return new (require('./run').Server)(_.extend({
      hostname: hostname
    }, _pick(prg, 'watch', 'service', 'port', 'interval', 'delay')));
  });
